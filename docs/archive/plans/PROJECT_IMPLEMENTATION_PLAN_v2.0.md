...
| 模块 4 | 动态地图尺寸与地图方案重构 | 进行中（profile 元数据、尺寸预检、受保护保存、草稿越界收口、内置/自定义 profile 首版已完成） | 已补地图 profile 元数据、当前方案、尺寸编辑前置条件展示、目标尺寸变更预检、通过预检后的受保护尺寸保存、尺寸变化后的基础越界收口，以及内置地图 profile 切换与自定义 profile 保存/删除/再次应用；现在每个地图 profile 也能直接触发尺寸预检并显示 ready/blocked 反馈，预检对象也会联动到下方尺寸预检区，阻塞 profile 还会在卡片里直接显示主要原因。日文 locale 也已从临时英文 fallback 恢复为真实日文覆盖层。后续仍需继续扩大前端运行时尺寸适配范围，并推进更完整的地图方案持久化。 |
...
### 当前已落地的低风险第一步
- 已新增地图 profile 元数据接口：`GET /status/map/profiles`
- 已新增地图尺寸变更预检接口：`GET /status/map/resize-precheck`
- 已新增地图尺寸受保护保存接口：`POST /status/map/resize`
- 已在地图设置面板中展示：
  - 当前地图方案
  - 地图方案列表
  - 尺寸编辑前置条件状态
...
- 当前已开放“通过预检后的受保护尺寸保存”，但还未进入高风险的自由地图编辑器阶段
