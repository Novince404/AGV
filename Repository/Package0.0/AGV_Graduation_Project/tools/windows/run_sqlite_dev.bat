...
start "AGV Backend SQLite" cmd /k "cd /d %PROJECT_ROOT%\backend && call .\venv\Scripts\activate && set AGV_DATA_BACKEND=sqlite && set AGV_DATABASE_URL=%SQLITE_DB% && set AGV_DATABASE_AUTO_CREATE=true && python -m uvicorn main:app --reload"

rem Start frontend
start "AGV Frontend" cmd /k "cd /d %PROJECT_ROOT%\frontend\agv-frontend && npm run dev"
