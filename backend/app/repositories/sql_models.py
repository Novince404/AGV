...
class UiSettingsEntity(Base):
    ...
    follow_distance: Mapped[float] = mapped_column(Float, nullable=False, default=0.75)
    deadlock_timeout_sec: Mapped[float] = mapped_column(Float, nullable=False, default=4.5)
...
class OIDCLinkRequestEntity(Base):
    __tablename__ = "oidc_link_request"
    ...
    user_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("auth_user.id"), nullable=True, index=True)
...
class RuntimeCommandEntity(Base):
    __tablename__ = "runtime_command"
...
class SchedulerLeaseEntity(Base):
...
class IdempotencyRecordEntity(Base):
    ...
    __table_args__ = (UniqueConstraint("scope_key", "idempotency_key", name="uq_idempotency_scope_key"),)
