...
def _minimum_password_length() -> int:
    return 12 if get_settings().app_environment in {"trial", "production"} else 8
...
def _build_authenticated_payload(user, session: AuthSession, raw_token: str | None = None) -> dict:
    ...
    if raw_token:
        payload["session_token"] = raw_token
    return payload
...
def resolve_request_actor(request: Request) -> dict:
    ...
    user, _session, _raw_token = resolved
    return {
...
def require_authenticated_actor(
    ...
    *,
    ...
    if actor.get("must_change_password") and not allow_password_change:
        raise_api_error(403, "auth_password_change_required")
    return actor
...
def _resolve_current_session(request: Request, touch: bool = True) -> tuple[object, AuthSession, str] | None:
    ...
    now = utc_timestamp()
    idle_ttl = max(int(get_settings().auth_session_idle_ttl_sec), 300)
    if int(session.expires_at) <= now or now - int(session.last_seen_at) > idle_ttl:
...
def login(username: str, password: str, client_key: str = "unknown") -> dict:
    normalized_username = str(username or "").strip()
    normalized_password = str(password or "")
    if not normalized_username or not normalized_password:
        raise_api_error(400, "auth_credentials_required")

    rate_key = f"{normalized_username.casefold()}:{str(client_key or 'unknown')}"
    retry_after = login_rate_limiter.retry_after(rate_key)
    if retry_after:
        raise_api_error(429, "auth_rate_limited", retry_after=retry_after)
    ...
    if user is None or not verify_password(normalized_password, user.password_hash):
        retry_after = login_rate_limiter.record_failure(rate_key)
        if retry_after:
            raise_api_error(429, "auth_rate_limited", retry_after=retry_after)
    ...
    event_broker.publish(
        ...
        scope_key=str(user.organization_id or user.id),
        entity_type="user",
        entity_id=user.id,
        data={"username": user.username, "role": user.role},
    )
    return _build_authenticated_payload(user, session, raw_token)


def create_external_session(user_id: str) -> dict:
    user = get_user_by_id(str(user_id or "").strip())
    if user is None:
        raise_api_error(404, "auth_user_not_found")
    _raise_if_login_restricted(user)
    remove_sessions_for_user(user.id)
...
def logout(request: Request) -> dict:
    ...
    if token:
        remove_session(hash_session_token(token))
...
def get_current_auth(request: Request) -> dict:
    ...
    if resolved is None:
        return _guest_payload()
    user, session, _raw_token = resolved
    return _build_authenticated_payload(user, session)


def change_password(request: Request, current_password: str, new_password: str) -> dict:
    resolved = _resolve_current_session(request, touch=False)
    if resolved is None:
        raise_api_error(401, "auth_login_required")
    user, _session, _raw_token = resolved
    ...
    if not current or not verify_password(current, user.password_hash):
        raise_api_error(401, "auth_invalid_credentials")
    if len(replacement) < _minimum_password_length():
        raise_api_error(400, "personal_register_password_invalid")
    if current == replacement:
        raise_api_error(400, "auth_password_reuse_not_allowed")
    ...
    record_operation_audit(
        ...
        actor={"id": user.id, "username": user.username, "role": user.role, "authenticated": True},
...
def register_enterprise(
    ...
    if len(normalized_password) < _minimum_password_length():
        raise_api_error(400, "personal_register_password_invalid")
...
def delete_operation_feed_item(request: Request, audit_id: int) -> dict:
    ...
    # Audit trails are append-only in the enterprise trial profile. Retention
    # may be implemented by a documented archival job, never by an API delete.
    raise_api_error(405, "audit_events_immutable", audit_id=int(audit_id))
