...
Database structure is maintained by Alembic; normal repository reads and
writes no longer create tables or add columns. For a persistent database, run
the maintenance commands from `backend/` before serving a trial deployment:
...
SQLite upgrades create a consistent backup with a SHA-256 checksum first. For
MySQL, create and verify an external backup, then use:

```powershell
python agv.py database upgrade --backup-confirmed
```

`AGV_DATABASE_AUTO_CREATE=true` remains a demo/development convenience: it
runs the same Alembic upgrade path at startup when the schema is not current.
Leave it disabled for trial and production deployments. See
[`docs/deployment/DATABASE_MIGRATIONS.md`](../docs/deployment/DATABASE_MIGRATIONS.md)
for backup, restore, and legacy v2/beta migration procedures.
...
# demo/development only; runs Alembic migrations
...
# run `python agv.py database upgrade --backup-confirmed`
AGV_DATABASE_AUTO_CREATE=false
