...
export function useAuthSession(options) {
  ...
  function buildAuthHeaders(baseHeaders = {}) {
    ...
    // agv_session is HttpOnly and therefore deliberately unavailable to JavaScript.
    // The separate non-secret CSRF cookie binds unsafe requests to this browser session.
    const csrfToken = readCookie('agv_csrf')
