...
## v3.0.0-beta.2 - 2026-07-18

### Added
- Flattened the repository so `backend/`, `frontend/`, `demo/`, `docs/`, `enterprise_client/`, and `tools/` are direct project entry points.
- Added a single root `VERSION` source, reproducible Python runtime/development lock files, and CI for backend lint/tests, SQLite/MySQL migrations, frontend type checks/tests/builds, dependency auditing, and secret scanning.
- Added FastAPI application factory/lifecycle boundaries, `/api/v1` resource endpoints, health/version/metrics endpoints, Problem Details errors, request IDs, and durable SSE event streaming.
- Added Alembic migrations, database check/backup/upgrade/verify/restore commands, SQLite integrity checks, and legacy database baseline recognition.
- Added Argon2id password storage, opaque HttpOnly sessions, CSRF protection, login rate limiting, first-login password changes, local recovery-admin bootstrap, and generic OIDC/Keycloak approval flow.
- Added deterministic scheduler/device boundaries, database leases, durable runtime commands/events, idempotency keys, and a simulation-only device adapter.
- Added a router/Pinia/TypeScript application shell, cookie-aware API client, role-aware navigation, and a first-login password-change page while preserving the existing dispatch workspace during gradual migration.
- Added Docker Compose trial deployment, MySQL/Keycloak examples, Windows API/scheduler split launchers, and updated PyInstaller packaging assets.

### Changed
- Moved normal schema changes out of repository request paths and into versioned migrations.
- Made demo users opt-in to the `demo` profile and documented the difference between local demos and enterprise trial deployments.
- Updated public README files, contribution/security/community documents, deployment guidance, and the PolyForm Noncommercial use boundary.

### Verified
- Backend compilation, Ruff, 17 pytest API/runtime/security/migration tests, and the SQLite smoke suite.
- Frontend TypeScript check, ESLint, Vitest, production build, and enterprise avoidance smoke test.
- Dependency lock resolution, `pip check`, `pip-audit`, Markdown link checks, sensitive-data scan, and migration/maintenance command checks.

### Known limitations
- This remains a simulation and enterprise-trial beta: it has no real AGV/PLC/MQTT/serial adapter, functional-safety certification, or high-availability scheduler cluster.
- Windows package scripts were validated through their Python entry points; a fresh PyInstaller executable build was not produced in this environment.
- Full backend mypy coverage is not enabled yet because legacy modules have existing typing debt. Ruff and automated runtime/API tests are enforced now.
- The original beta.1 baseline migration must be converted to a fully static schema snapshot before declaring the v3 line stable; use verified backups before upgrading any non-demo database.
